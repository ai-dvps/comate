...
export default class SmartsheetAddFields extends BaseDocCommand {
  static override description = 'Add fields (columns) to a smartsheet';
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(SmartsheetAddFields);
    if (flags.json) {
      const body = JSON.parse(flags.json) as Record<string, unknown>;
      await this.callDocTool('smartsheet-add-fields', body);
      return;
    }
