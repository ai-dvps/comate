...
Comate 内置定时任务：任务绑定工作区，支持一次性延迟与周期调度，可在 UI 配置，也可在聊天中由 AI 直接创建（创建即生效）。每次触发以免审批模式启动一个全新会话，首条消息为系统包装的 /goal（指令 + 完成标准 + 轮次上限）。结果经桌面通知、应用内标记与可选 WeCom 摘要触达；UI 入口在右侧标题栏，全局任务面板内可查看每个任务的执行历史并跳进对应会话。
...
- **创建直接生效，无确认环节。** 所有渠道（UI 表单、本地聊天、WeCom 等远程入口）创建的定时任务创建即 active；快照在创建时写入。用户在冒烟后认为确认步骤多余并明确接受远程渠道的风险。(session-settled: user-directed — chosen over 草稿+UI 确认: 该决定推翻了之前的"创建环节是唯一安全闸门"，用户确认全渠道移除)
...
- A3. 聊天中的 AI 助手：理解自然语言请求，把当前上下文整理为自包含指令，调用任务创建工具。
- A4. 远程入口用户（如 WeCom 联系人）：可请求创建任务，与本地创建同权直接生效。
...
- R5. 用户在聊天中可用自然语言请求创建、查看、暂停任务；AI 通过专用 MCP 工具直接创建，任务创建即生效。
- R6. 聊天创建的任务与 UI 创建同权：创建即生效，执行会话可见，面板与未读事件同步。
...
- F2. 聊天创建任务
  ...
  - **Actors:** A1, A3
  - **Steps:** AI 将当前上下文整理为自包含指令并调用创建工具 → 任务创建即生效 → 面板未读提示。
  - **Covered by:** R5, R6, R7
- F3. 到点执行与触达
  - **Trigger:** 调度器发现任务到期。
  - **Actors:** A2, A1
...
- AE4. **Covers R6.** WeCom 联系人请机器人"明早 9 点跑一次测试"，任务创建即生效并出现在面板；标题栏未读徽标提示 A1 有新任务产生。
