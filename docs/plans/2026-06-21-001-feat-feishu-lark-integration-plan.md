...
- **Card callbacks are served by a dedicated Express route.** Because the Chat SDK adapter does not expose card action handling, `/api/feishu/card/:workspaceId` uses `@larksuiteoapi/node-sdk` `CardActionHandler` configured with the workspace's own `feishuEncryptKey` and `feishuVerificationToken`. Each workspace's Feishu app configures this URL with its own workspace segment so signature verification uses the correct credentials.
...
**Approach:**
- Add `/api/feishu/card/:workspaceId` mounted before the global `express.json()` middleware so the `@larksuiteoapi/node-sdk` handler can read the raw request body for signature verification. Each workspace's Feishu app configures its card callback URL with its own `workspaceId` segment so signature verification uses that workspace's credentials.
- Use `CardActionHandler` from `@larksuiteoapi/node-sdk`, configured with the workspace's `feishuEncryptKey` and `feishuVerificationToken` identified by `:workspaceId`. Verify the workspace still exists and has `feishuBotEnabled` before processing the action.
...
| Card callbacks require a publicly reachable callback URL while the app runs as a desktop sidecar | Document that the card callback URL must point to the local server URL (`http://localhost:<port>/api/feishu/card/:workspaceId`) and that Feishu must be able to reach it (e.g., via a tunnel in development or the deployed endpoint in production). |
| `@larksuiteoapi/node-sdk` signature verification requires raw body; global `express.json()` would break it | Mount `/api/feishu/card/:workspaceId` before the global JSON middleware or use route-specific raw-body middleware. |
