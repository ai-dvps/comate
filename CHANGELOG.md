...
- **Claude Code settings are grouped with their Agent** — Agent choices now use a compact grouped-list layout with clearer selection and focus states, while Output style appears directly inside the Claude Code row with the same touch-friendly selector styling as the rest of Settings.
