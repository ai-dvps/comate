...
- **Feishu bot menu `/resume` and `/new` keys** — Feishu bot menu events configured with a leading slash (e.g. `/resume`, `/new`) are now normalized and handled the same as `resume` and `new`. Previously, a menu key of `/resume` was treated as unknown and either silently failed or replied with "未知的菜单操作". Additionally, menu events are now handled on the WebSocket/long-connection path used by the chat adapter, not only on the HTTP callback route, so they work when the Feishu app is configured to use long-connection event subscription. Diagnostic logging now records request arrival, the raw menu payload, normalized key, chosen branch, and handling result to make future menu configuration issues easier to trace.
