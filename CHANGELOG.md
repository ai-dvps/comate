...
- **Output style selector in the prompt toolbar** — Sessions can now switch Claude Code's output style (Default, Explanatory, Learning, Concise, plus any workspace-custom styles the CLI reports) from a dedicated selector next to Fast mode; the choice persists per session, applies to the next turn, and is also selectable when starting a new chat.
