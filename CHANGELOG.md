...
- **Git Changes panel** — a new collapsible right sidebar shows the active workspace's changed files in two clearly separated sections (untracked and changed), each with its own icon header and file count, and supports tree and flat views. Double-clicking a file opens its diff in a dedicated `GitDiffPanel` between the chat panel and the Git Changes panel, with tabs for multiple diffs, unified/side-by-side toggle, and syntax highlighting for untracked files. The panel widths and collapsed state persist across app restarts.
...
- **Git diff view now uses `git-diff-view`** — the diff panel that opens when double-clicking a changed file now renders with `@git-diff-view/react` instead of the previous plain code block, giving GitHub-style unified and split views with syntax highlighting, line numbers, and hunk headers. Untracked, binary, and deleted files keep their existing specialized rendering.
