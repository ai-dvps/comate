...
- **Git Changes panel** — a new collapsible right sidebar shows the active workspace's changed files, groups untracked files at the top, supports tree and flat views, and opens a read-only diff view on double-click. Diff rendering defaults to unified inline with a side-by-side toggle, untracked files render with syntax highlighting, and the panel width/collapsed state persist across app restarts.
