...
- **Result-focused message panel flicker, overlap, and missing history** — Result Focus now keeps one virtual-list shell for short and long sessions, projects assistant turns with source-derived stable region identities, and preserves unchanged text and process ghosts while streaming updates only the affected region. Historical pagination now uses normalized range boundaries, merges safely with live tail updates, and restores the exact visible scroll anchor when older pages load, preventing full-list refreshes, overlapping rows, and sessions that showed only the latest process steps.
