...
- **Settings footer refactor** — removed the global Save/Cancel footer from the Settings panel. The General and Workspace tabs now show their own local Save/Cancel footers only when their own state is dirty; Appearance remains auto-save; Provider and Bot tabs keep their existing per-section controls. The close-guard unsaved-changes dialog is now a shared `UnsavedChangesDialog` component, and switching workspaces inside the Workspace tab is also guarded when there are unsaved changes.
