...
- **Embedded browser: allow insecure certificates (global setting)** — the embedded browser now loads private-CA / hostname-mismatched internal sites by default (it is headless Chrome for Testing with no cert-warning interstitial, so such sites otherwise hard-fail as `ERR_BLOCKED_BY_CLIENT`). Controlled by a new app-global setting in Settings → General, persisted server-side and applied to every spawned Chrome as `--ignore-certificate-errors`; on by default, turn it off there for strict certificate validation. (Replaces the earlier `COMATE_BROWSER_IGNORE_CERT_ERRORS` env var, which is now a no-op.) Applies to the embedded browser only.
