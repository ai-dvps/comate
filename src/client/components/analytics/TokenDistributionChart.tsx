/**
 * TokenDistributionChart — donut + legend for the input/output/cache split.
 * Ported from the reference app; consumes comate's camelCase TokenDistribution
 * shape (input/output/cacheCreation/cacheRead).
 */

import React, { useMemo } from 'react'
import { useTranslation } from 'react-i18next'
import { Database, Eye, TrendingUp, Zap } from 'lucide-react'
  ...
  const items = useMemo(
    () => [
      ...
      },
      {
        ...
        color: 'hsl(var(--color-metric-purple))',
        icon: Zap,
      },
      {
        label: t('cacheCreation'),
        value: distribution.cacheCreation,
        color: 'hsl(var(--color-metric-blue))',
        icon: Database,
      },
      {
        label: t('cacheRead'),
        value: distribution.cacheRead,
        color: 'hsl(var(--color-metric-amber))',
        icon: Eye,
      },
    ],
    [t, distribution],
    ...
    <div className="space-y-5">
      <div className="relative flex items-center justify-center">
            ...
            <Tooltip key={arc.label}>
              ...
              <ChartTooltip
                ...
                rows={[
                  ...
                  { label: t('tooltip.share'), value: `${(arc.percentage * 100).toFixed(1)}%` },
                ]}
              />
            </Tooltip>
          ))}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
          ...
          <div className="text-[9px] font-medium text-text-tertiary uppercase tracking-wider mt-1">
            {t('totalTokenUsage')}
          </div>
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                'transition-colors duration-200 hover:bg-surface-hover/30',
              )}
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between mb-1">
                  <span className="text-[11px] font-medium text-text-primary/80 truncate">
                    {item.label}
                  </span>
                  <span className="font-mono text-[10px] text-text-tertiary ml-2 tabular-nums">
                    {percentage.toFixed(1)}%
                  </span>
                </div>
                <div className="h-1 bg-surface-hover/30 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${percentage}%`, backgroundColor: item.color, opacity: 0.8 }}
                  />
                </div>
              </div>
