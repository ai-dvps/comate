    ...
    <label className="text-[11px] font-medium text-text-tertiary">{t('providers.authToken')} *
      <span className="mt-1 flex gap-2">
        ...
        <button
          ...
          title={show ? t('providers.hideToken') : t('providers.showToken')}
...
export default function ProviderSection() {
  ...
  const requestDelete = async (providerId: string) => {
    setDeleteImpactLoadingId(providerId)
    const result = await store.getDeleteImpact(providerId)
    setDeleteImpactLoadingId(null)
    if (!result.ok) return
    setDeleteImpact(result.affectedSessionCount ?? 0)
    ...
    return (
      <section className="max-w-4xl p-4 sm:p-6" aria-labelledby="provider-editor-title">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={resetEditor} aria-label={t('actions.cancel')} title={t('actions.cancel')} className="rounded-md p-1.5 text-text-tertiary hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"><XCircle className="h-4 w-4" /></button>
        </div>
