...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('Failed to render Markdown preview:', error, info)
  }
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex size-full items-center justify-center px-6 text-center text-sm text-text-secondary" role="alert">
          {this.props.fallback}
        </div>
      )
    }
...
export default function MarkdownPreview({ content, className, style }: MarkdownPreviewProps) {
  const { t } = useTranslation('common')

  return (
    <div className="size-full" style={style}>
      <MarkdownPreviewErrorBoundary
        fallback={t('markdownPreviewError')}
        resetKey={content}
      >
        <Streamdown
          className={cn(
