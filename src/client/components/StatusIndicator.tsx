...
export default function StatusIndicator({ state, count, className }: StatusIndicatorProps) {
  ...
  if (count === undefined) {
    return (
      <span title={title} className={cn('inline-flex flex-shrink-0', className)}>
        {icon}
      </span>
    )
  }
  ...
  return (
    <span className={cn('inline-flex items-center gap-0.5', className)} title={title}>
      {icon}
