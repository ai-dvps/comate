...
const { settingsState, soundCalls } = vi.hoisted(() => ({
  ...
  soundCalls: { attention: 0, completion: 0 },
}))
...
vi.mock('../lib/sound-player', () => ({
  playSound: (kind: 'attention' | 'completion') => {
    soundCalls[kind] += 1
  },
}))
...
describe('useNotificationSounds', () => {
  beforeEach(() => {
    settingsState.notificationSoundsEnabled = true
    soundCalls.attention = 0
    soundCalls.completion = 0
    resetStore()
    vi.useFakeTimers()
  })
  ...
  it('covers AE2/AE3: a new approval/question requestId plays attention once', () => {
    ...
    expect(soundCalls.attention).toBe(0)
    act(() => enqueue('s1', 'req-1'))
    expect(soundCalls.attention).toBe(1)
  })
