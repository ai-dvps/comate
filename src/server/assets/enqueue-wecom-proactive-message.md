---
name: enqueue-wecom-proactive-message
description: Queue a WeCom message to be delivered to another user's session. Use when the user wants to send a message to someone else, notify another user, or proactively reach out via WeCom.
---

<objective>
Queue WeCom proactive messages using the `wecom queue enqueue` CLI command. This ensures the message appears in the recipient's session history so they can reply in context.
</objective>

<quick_start>
Queue a message to another user:
...
4. **Report result**: Show the command executed and the queue entry ID returned by the CLI
</workflow>
...
<output>
Queueing proactive message to ZhangSan:
...
<output>
Queueing proactive notification to LiSi:
...
<output>
Queueing proactive message to WangWu:
...
<pitfall name="using_direct_send">
...
- Results (success or error) are reported clearly, including the queue entry ID on success
