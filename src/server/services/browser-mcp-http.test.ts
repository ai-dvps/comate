...
describe('browser-mcp-http (U6)', { concurrency: false }, () => {
  ...
  it('answers initialize with server info', async () => {
    ...
    assert.match(res.json.result.instructions, /Mutation tools return only bounded receipts, never a page model/i);
    assert.match(res.json.result.instructions, /After every mutation, call getPageState/i);
    assert.match(res.json.result.instructions, /getPageState[\s\S]*findElements[\s\S]*getElementDetails[\s\S]*act[\s\S]*upload[\s\S]*activate[\s\S]*takeScreenshot/);
  ...
  it('lists the browser tool surface', async () => {
    ...
    for (const expected of ['open', 'getPageState', 'getDecisionObservation', 'rebindVisualCandidates', 'getTaskState', 'startTask', 'proposeTaskEvidence', 'abandonTask', 'recheckTaskOutcome', 'abandonOutcomeTracking', 'acknowledgeDuplicateRisk', 'findElements', 'getElementDetails', 'act', 'upload', 'activate', 'takeScreenshot', 'submit', 'extract', 'authenticatedRequest', 'requestHandoff', 'close']) {
