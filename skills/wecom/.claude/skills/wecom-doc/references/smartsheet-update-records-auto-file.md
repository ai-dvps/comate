# smartsheet-update-records-auto-file API

Update records in a smartsheet with automatic file upload. Any `image_path` or `file_path` fields in the records are resolved server-side into WeCom media IDs.

## CLI usage

```bash
wecom doc:smartsheet-update-records-auto-file [flags]
```
...
```bash
wecom doc:smartsheet-update-records-auto-file --docid DOCID --sheet-id SHEET --data '{
  "records": [
    {"record_id": "rec_1", "field_values": {"fld_attachment": {"file_path": "docs/scan.pdf"}}}
  ]
}'
```
